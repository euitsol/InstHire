    ...
    <div class="mb-4 card">
        <div class="card-body">
            <div class="d-flex align-items-center justify-content-between">
                <h2 class="mb-4 card-title">{{ __('Edit Stall Option') }}</h2>
            ...
            <form action="{{ route('institute.setup.jfs.update', $stallOption->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-control">
                        <option value="{{ App\Models\JobFairStallOption::STATUS_ACTIVE }}" {{ old('status', $stallOption->status) == App\Models\JobFairStallOption::STATUS_ACTIVE ? 'selected' : '' }}>Active</option>
                        <option value="{{ App\Models\JobFairStallOption::STATUS_INACTIVE }}" {{ old('status', $stallOption->status) == App\Models\JobFairStallOption::STATUS_INACTIVE ? 'selected' : '' }}>Inactive</option>
                    </select>
