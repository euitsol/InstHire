...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('institute.job-post.update', $jobPost->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row g-4">
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6" id="employee_select" style="display: {{ old('type', $jobPost->type) == App\Models\JobPost::TYPE_SELF ? 'block' : 'none' }}">
                                        ...
                                        <select name="employee_id" class="form-select @error('employee_id') is-invalid @enderror">
                                            ...
                                            @foreach($employees as $employee)
                                                <option value="{{ $employee->id }}" {{ old('employee_id', $jobPost->employee_id) == $employee->id ? 'selected' : '' }}>{{ $employee->name }}</option>
                                            @endforeach
                                        </select>
