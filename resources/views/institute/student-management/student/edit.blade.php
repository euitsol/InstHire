    ...
    <div class="card mb-4">
        <div class="card-body">
            <div class="d-flex align-items-center justify-content-between">
                <h2 class="card-title mb-4">{{ __('Student Edit') }}</h2>
                <a href="{{ route('institute.student.index') }}" class="btn btn-sm btn-primary">
                    <i class="bi bi-plus"></i> {{ __('Back') }}
                </a>
            </div>

            <form action="{{ route('institute.student.update', $student->id) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="mb-3">
                    <label for="name" class="form-label">Name</label>
                    <input type="text" class="form-control" value="{{ old('name', $student->name) }}"
                    ...
                    <select name="session_id" id="session_id" class="form-control">
                        ...
                        @foreach ($sessions as $session)
                            <option value="{{ $session->id }}"
                                {{ old('session_id', $student->session_id) == $session->id ? 'selected' : '' }}>
                                {{ $session->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="{{ $student->roll }}" placeholder="Enter roll"
                        id="roll" name="roll">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="{{ old('phone', $student->phone) }}"
